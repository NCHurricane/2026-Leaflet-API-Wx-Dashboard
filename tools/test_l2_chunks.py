...
SITE = sys.argv[1].upper() if len(sys.argv) > 1 else "KRAX"
...
def main():
    ...
    for prefix in sorted_prefixes[-3:]:
        chunks = scans[prefix]
        complete = _is_scan_complete(chunks)
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        filename = _scan_assembled_filename(test_prefix)
