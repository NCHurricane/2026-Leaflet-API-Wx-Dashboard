...
MAPDATA_BASE_URL = "https://www.wpc.ncep.noaa.gov/NationalForecastChart/mapdata"

# Fixed colors for single-Feature precip layers (no embedded color in source).
SURFACE_PRECIP_COLORS: dict[str, str] = {
    "qpf": "#4169E1",
    "trw": "#8B00FF",
    "sno": "#ADD8E6",
    "rsn": "#DA70D6",
...
def _surface_vector_product(
    ...
    return {
        ...
        "days": (day,),
        "label": label,
        "url": f"{MAPDATA_BASE_URL}/{filename}",
        "format": fmt,
        "cache_path": f"surface/{product_id}.json",
        "seasonal": False,
        "color": color,
...
SURFACE_VECTOR_PRODUCTS = tuple(
    product
    for day in (1, 2, 3)
    for product in (
        _surface_vector_product(
            f"surface_qpf_d{day}", f"Rain — Day {day}",
            f"qpfD{day}.json", day, "geojson_single", SURFACE_PRECIP_COLORS["qpf"],
        ),
        _surface_vector_product(
            f"surface_trw_d{day}", f"Rain + Thunderstorms — Day {day}",
            f"trwD{day}.json", day, "geojson_single", SURFACE_PRECIP_COLORS["trw"],
        ),
        _surface_vector_product(
            f"surface_sno_d{day}", f"Snow — Day {day}",
            f"snoD{day}.json", day, "geojson_single", SURFACE_PRECIP_COLORS["sno"],
        ),
        _surface_vector_product(
            f"surface_rsn_d{day}", f"Rain/Snow Mix — Day {day}",
            f"rsnD{day}.json", day, "geojson_single", SURFACE_PRECIP_COLORS["rsn"],
        ),
        _surface_vector_product(
            f"surface_zr_d{day}", f"Freezing Rain — Day {day}",
            f"zrD{day}.json", day, "geojson_single", SURFACE_PRECIP_COLORS["zr"],
        ),
        _surface_vector_product(
            f"surface_ww_d{day}", f"Winter Weather — Day {day}",
            f"wwD{day}.json", day, "geojson_single", SURFACE_PRECIP_COLORS["ww"],
        ),
        _surface_vector_product(
            f"surface_ero_d{day}", f"Excessive Rainfall — Day {day}",
            f"ERODay{day}.geojson", day, "geojson_fc",
        ),
        _surface_vector_product(
            f"surface_swo_d{day}", f"Severe Weather Outlook — Day {day}",
            f"SWODay{day}.geojson", day, "geojson_fc",
        ),
        _surface_vector_product(
            f"surface_fwx_d{day}", f"Fire Weather — Day {day}",
            f"FWXDay{day}.geojson", day, "geojson_fc",
        ),
    )
)

SURFACE_PNG_PRODUCTS = SURFACE_ANALYSIS_PRODUCTS + SURFACE_FORECAST_PRODUCTS
