...
def _mark_profile_run_complete(
    ...
    if profile_key == "local-primary" and state_name != worker_name:
...
def _warm_job_rolling(
    ...
    cursor = max(int(start_offset or 0), guaranteed_latest)
    while cursor < len(newest_first):
        if _deadline_reached(deadline_utc):
            return totals, cursor, True
        frame = newest_first[cursor]
        stats = warm_frame_tiles_from_canvas(
            cache_root=_CACHE_ROOT,
            sat_id=sat_id,
            sector=sector,
...
def run_satellite_v2_worker(
    ...
    worker_name = "satellite_v2_meso" if meso else "satellite_v2"
    state_name = (
        worker_name if profile_key == "full" else f"{worker_name}_{profile_key}"
    )
